...
def cross_entropy_list(sources, targets, model, cache = None, batch=False, calculate=True):
    ...
    # inference: get logits from the model
    with torch.no_grad():
        input_ids = input_ids.to(device)
        logits = model(input_ids).logits.cpu()[:, :-1].contiguous()
        # multi-gpu
        # print("Outside: input size", input_ids.size(),
...
def inference_autobatch( model, encoder, example, batch = 1, prelog = False, cache = None):
    ...
    pred = {
            ...
            'tok_mean_logits': avg_cond_ce,
            'dcpmi' : dcpmi_pred,
            'dcpmi_logits': dcpmi,
            'pmi': pmi_pred,
            'pmi_logits': pmi,
            'domain_cond': lm_domain_cond_pred,
            'domain_cond_logits': domain_cond_ce,
           }
    return pred
...
def fwd(model, encoder, examples, batch, cache=None):
    ...
    sims = [ex["sim"] for ex in examples]
    ...
    for key in predictions_dict.keys():
        if not key.endswith("logits"):
            results[key] = sum(list(map(lambda v: v[0] == v[1], zip(predictions_dict[key] , labels) )))/len(labels)

    # save labels for later
    predictions_dict['labels'] = labels
    predictions_dict['sims'] = sims
    return results, predictions_dict


def compute_interpolate_metrics(preds, preds_interpolate, args):
    def compute_interpolate_metrics_single(logits, interpolate_logits, sim_score, args, m=None):
        max = True if m == "max" else False
        logits, interpolate_logits, sim_score = np.array(logits), np.array(interpolate_logits), np.array(sim_score)
        num_logits = logits.shape[-1]

        '''
        softmax
        '''
        sim_score = sim_score.reshape((-1, args.num_doc))
        sim_score = softmax(sim_score, axis=-1)
        sim_score = sim_score.reshape((-1, 1))
        interpolate_logits = interpolate_logits * sim_score
        interpolate_logits = np.reshape(interpolate_logits, newshape=(args.num_doc, -1, num_logits), order='F')
        interpolate_logits = interpolate_logits.sum(axis=0)

        '''
        mean
        '''
        # interpolate_logits = np.reshape(interpolate_logits, newshape=(args.num_doc, -1, num_logits), order='F')
        # interpolate_logits = interpolate_logits.mean(axis=0)

        # compute combined scores
        final_logits = (1 - args.interpolate_k) * logits + args.interpolate_k * interpolate_logits
        final_predict = np.argmax(final_logits, axis=1) if max else np.argmin(final_logits, axis=1)
        return final_logits, final_predict



    prob_list = {"lm": "min", "tok_mean": "min", "dcpmi": "max", "pmi": "max", "domain_cond": "min"}
    accs_final, preds_final = {}, {}
    for p, m in prob_list.items():
        final_logits, final_predict = compute_interpolate_metrics_single(preds[f"{p}_logits"], preds_interpolate[f"{p}_logits"], preds_interpolate["sims"], args, m=m)
        preds_final[p] = final_logits.tolist()
        # calculate accuracies
        accs_final[p] = sum(list(map(lambda v: v[0] == v[1], zip(final_predict.tolist(), preds["labels"]))))/len(preds["labels"])
    return accs_final, preds_final


def score(model, model_name, encoder, examples, examples_interpolate, stem, split, batch, args):
    def load_cache(split):
        hist_path = f'{args.output_dir}/{model_name}-{split}.hist'
        ...
        return cache, hist_path

    def save_cache(cache, hist_path):
        print('=' * 50)
    ...
    def write_acc(stem, split, interpolate_k):
        # save scores
        ...
        with open(results_path, 'w') as out:
            ...
            out.write("interpoalte: \n")
            out.write(json.dumps(accs_interpolate))

            out.write("final: \n")
            out.write(json.dumps(accs_final))

            if interpolate_k == 0.1:
                logger.info(f"original: {accs}")
                logger.info(f"interpolate: {accs_interpolate}")
            logger.info(f"final: {accs_final}")

    def write_pred(stem, split, preds):
        # save predicted labels
        preds_path = f'{stem}/{split}.preds'
        with open(preds_path, 'w') as out:
            out.write(json.dumps(preds))

    cache, hist_path = load_cache(split)
    cache_interpolate, hist_path_interpolate = load_cache(f"{split}_interpolate")

    accs, preds = fwd(model, encoder, examples, batch, cache)
    print(accs)
    # accs_interpolate, preds_interpolate = fwd(model, encoder, examples_interpolate, batch, cache_interpolate)

    save_cache(cache, hist_path)
    # save_cache(cache_interpolate, hist_path_interpolate)


    # for args.interpolate_k in np.arange(0.1, 1, 0.1):
    #     args.interpolate_k = np.round(args.interpolate_k, 2)
    #     args.output_dir = f"{args.dataset_dir}/{args.model}/sample{args.sample}/inter{args.interpolate_k}"
    #     Path(args.output_dir).mkdir(exist_ok=True, parents=True)
    #
    #     accs_final, preds_final = compute_interpolate_metrics(preds, preds_interpolate, args)
    #
    #     logger.info(f"--------inter: {args.interpolate_k}--------------")
    #     write_acc(args.output_dir, split, args.interpolate_k)
    #     write_pred(args.output_dir, split, preds)
    #     write_pred(args.output_dir, f"{split}_interpolate", preds_interpolate)
    #     write_pred(args.output_dir, f"{split}_final", preds_final)
