...
class EvaluatingWrapper():
    def __init__(self, model, encoder, examples, knn_dstore, args):
        self.model = model
    ...
    def load_cache_data(self):
        self.hist_path = f'{self.args.output_dir}/{self.args.model}-{self.args.split}.hist'
    ...
    def get_knn_scores(self, outputs, all_lens, targets, gold_labels=None, sources=None):
        ...
        for i, t in enumerate(targets):
            ...
            print(f"label: {gold_labels[i]} | ", f"{label_num_list}")
            # print("debug t: ", t)
    ...
    def cross_entropy_list(self, sources, targets, gold_labels, cache=None, batch=False, calculate=True):
        ...
        if cache is not None:
            ...
            for knn_lambda in np.linspace(0, 1, 11):
        ...
        with torch.no_grad():
            ...
            full_knn_scores, label_knn_scores = self.get_knn_scores(outputs, all_lens, targets, gold_labels=gold_labels, sources=sources) # 10xvocab size
